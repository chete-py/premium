...
url = "https://docs.google.com/spreadsheets/d/1evGpco7ldQaHNF7S2_vB_P5lXSIEtnx3Ctiwsiy0-NU/edit?gid=0#gid=0"
...
with view1:
    ...
    with tab1:
        ...
        if view == 'Renewal':
            ...
            loss_of_use = st.selectbox("Choose Loss Of Use Amount Charged", ["Inclusive", "Excluded", 1500, 3000, 4500, 6000])
            ...
            if st.button("Calculate"):
                ...
                <head>
                 <style>
                    ...
                    width: 45%;
                    margin: 1.5px auto; /* Center the table */
                    font-size: 10px;
                    font-family: Candara;
                }}
        ...
        if view == 'New':
            ...
            excess_protector = st.selectbox("Choose excess protector rate", ["Inclusive", "0.25%", "0.5%", "Excluded"])
            pvt = st.selectbox("Choose pvt rate", ["Inclusive", "0.25%", "0.5%", "Excluded"])
            loss_of_use = st.selectbox("Choose Loss Of Use Amount charged", ["Inclusive", "Excluded", 1500, 3000, 4500])
            policy_fee = st.selectbox("Choose cover", ["Renewal", "New Business",])
            notes1 = st.text_input("Include Important Remarks 1 eg. Political/Terrorism Risks/RSCC - Reinstated at 0.35% of Value Once utilized")
            notes2 = st.text_input("Include Important Remarks 2 eg.  Days Loss of use/Courtesy Car - Reinstated at KShs. 3,000/- Once Utilized")
            notes3 = st.text_input("Include Important Remarks 3 eg. Excess Protector - Own Damage Reinstated at 0.25% of Value Once utilized")
            ...
            if underwriter == 'APA INSURANCE':
                minimum += 42500
            elif underwriter == 'CANNON GENERAL INSURANCE':
                minimum += 37500
        
            if st.button("Calculate"):
                premium = max(value * (rate/100) * (days/365), minimum)
        ...
        if view == 'Comperative Quote':
            ...
            reg = st.text_input('Enter Registration')            
            value = int(st.number_input('Sum Insured')) 
            loss_of_use = st.selectbox("Choose Loss Of Use Amount charged", ["Inclusive", "Excluded", 3000])
            ...
            if st.button("Calculate"):
                ...
                <table>
                    <tr>
                        ...
                        <th colspan="2">APA INSURANCE</th>
                        <th colspan="2">FIDELITY</th>
                        <th colspan="2">ICEA LION</th>
                        <th colspan="2">GA INSURANCE</th>                        
                    </tr>
                    <tr >
                    ...
                    <tr>
                        ...
                        <td>{formatted_ga_premium}</td> <!-- Updated formatting for better readability -->
                    </tr>
                    ...
                    <tr>
                        ...
                        <td style="color:red">0.45%</td>
                        <td >{formatted_ga_levies}</td> <!-- Updated formatting for better readability -->
                    </tr>
                    <tr>
                        <td>Policy Fee</td>
                        <td></td>
...
with view3:
    ...
    with tab33:
        ...
        newdf['Count'] = 1
        renew_df['Count'] = 1
        renewals = renew_df[renew_df['Status'] == 'Renewed']
        ...
        fig.update_layout(title={'text': 'MONTHLY INDIVIDUAL NEW BUSINESS', 'x': 0.5, 'xanchor': 'center'},  width=650,
                                                xaxis_title='Month',
